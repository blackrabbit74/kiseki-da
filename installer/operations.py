...
def _activate_runtime(plugin: Path) -> None:
    """Load code and schemas from the selected runtime during repeated operations."""
    # Versioned runtime files are immutable; generated bytecode must not invalidate rollback hashes.
    sys.dont_write_bytecode = True
    loaded = sys.modules.get("core")
    current = getattr(loaded, "__file__", None)
    if current and Path(current).resolve().parent.parent != plugin.resolve():
    ...
    with tempfile.TemporaryDirectory(prefix="kiseki-da-installed-hook-smoke-") as raw:
        ...
        for host, ownership in host_ownership.items():
            ...
            cases = (
                ...
                ("user-input", {"hook_event_name": "UserPromptSubmit", "prompt": "今回の案件を確認して"}, None),
            ...
            for event_type in ("session_start", "user_input", "guard", "tool_call", "session_end"):
...
def doctor() -> tuple[int, dict[str, Any]]:
    ...
    if smoke_targets:
        try:
            ...
            add("installed-hook-smoke", "ok", "6 hook routes / disposable state")
