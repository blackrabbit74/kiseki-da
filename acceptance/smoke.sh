...
WS="$(mktemp -d)"            # 作業ディレクトリは state の外（state 配下の書込は deny されるため）
TMP="$(mktemp -d)"
FAIL=0
pass() { echo "PASS $1"; }
...
$CTX task set demo-auth --add-criterion "更新処理の単体テストが通る :: pytest tests/test_auth.py" >/dev/null || fail "C2b add C1"
$CTX task set demo-auth --add-criterion "変更差分を読み返した :: Read git diff" >/dev/null || fail "C2c add C2"
...
$CTX approve "$CID" --review-by 2027-03-01 >/dev/null && pass "C6b approve $CID" || fail "C6b approve"
