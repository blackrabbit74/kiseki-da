...
**状態:** 2026-09-05の利用者による最新版のGitHub push・任意ディレクトリ利用の依頼を受け、監査・配布検証後に公開する。
...
|---|---|---|---|
| D-01 | Windows 11 native | 検証待ち。対応対象外 | Windows 11実機でinstall/update/rollback/uninstall、6 hooks、trust、空白・日本語path、並列logを通過したとき |
...
| D-09 | 最終公開承認 | 唯一のrelease blocker | 利用者の明示的な公開依頼を対象commit・tree・asset hashに結び付けて記録したとき |
...
4. HEADを`v0.1.0-beta.1`でtag付けし、公開予定一覧とhashを提示する。
