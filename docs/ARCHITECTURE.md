...
- 進行中カード: 400 token以下
- 環境情報: 150 token以下
- 予算超過時: goals、自由記述、persona任意項目、preferences、cards、constraintsの順に省く
...
すべての変更操作は preflight → preview → backup → staging → apply → verify → commit の順で行う。host inventoryと設定hashはlock取得後にも再照合し、installed plugin cacheの5 hookを一時stateでsmokeする。失敗は逆順rollbackし、未完了journalがある間は別変更を開始しない。通常uninstallは個人状態を残す。
