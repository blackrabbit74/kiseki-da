...
- 新しいカードの `workspace` はセッションを開始した案件フォルダ。常駐へは同じフォルダのカードだけが入る。所属未設定の旧カードは自動注入せず、必要なら `kiseki-da task set <id> --workspace "<案件フォルダ>"` で所属を明示する。
...
| C2 | 変更差分を読み返した | Read /path/to/diff.txt | - | open |
...
ヘッダは `id` / `status`（open | done | deferred | abandoned）/ `risk`（R0..R3）/ `kind`（code | research | decision | ops | writing）/ `created` / `updated` / `budget`（`k=v` 空白区切り）/ 任意 `workspace`（案件フォルダの絶対パス）。上の雛形は workspace がない旧形式。ヘッダにコメントは書かない。

## 完備規則
- Goal は利用者の言葉で書き、理由（なぜやるか）を添える。
- 完了条件は claim（何が真になるか）＋ check（それを確かめるツールかコマンド）。check は証拠を生むツール名かコマンド名で書き始める（`pytest tests/x.py`、`Read /path/to/diff.txt`、`Grep <pattern>`、`curl -i <url>`）。散文の check（「動作を確認する」）は照合できない。
...
## v2.4 の補足
- `kiseki-da task set <id> --constraint "必須制約・対象外" --next "次の作業"` を使う。next_action は任意ヘッダで保存する。
- check は引数まで一致させる。`last` が他セッションの結果を自動で拾うことはない。
- 保留は `task defer --reason` に限る。riskを下げて検証を通すことはできない。
- 現在の明示指示による実行は `task override --command "操作" --quote "最新の利用者発言全文"`。未検証・リスクを消す操作ではない。
- カード全文の長さを理由に常駐から丸ごと削除しない。常駐は要点と取得先、詳細はカードに保持する。
